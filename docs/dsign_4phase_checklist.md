...
**D2.5:** schedule API (`/api/schedule/*`, `return-to-schedule`) — session **или** Bearer.
...
**Спека:** [dsign_schedule_spec.md](./dsign_schedule_spec.md) (v1.5)

| ID | Задача | Статус | PR |
|----|--------|--------|-----|
| D2.1 | Модель `ScheduleRule`, CRUD API, `/week`, `/now` | ✅ | #103 |
| D2.2 | `ScheduleEngine`, playback source, `return-to-schedule` | ✅ | #104 |
| D2.3 | UI вкладка «Расписание», drag/resize, panel | ✅ | #105 |
| D2.4 | Timezone + NTP в Settings, SIGTERM hook | ✅ | #106 |
| D2.5 | Month view, exceptions, batch API, Bearer, touch/keyboard | ✅ | #107 |

**Acceptance:**
- [x] Правило → playlist (±30 s) — плеер: `/schedule/now`, `source=schedule`
- [ ] Offline 24 ч — расписание работает 🟡 (timezone/NTP в Settings; формальный тест не прогнан)
- [x] Skip-day для weekly/monthly (`POST /api/schedule/exceptions`)
- [ ] Fleet Bearer на schedule — код ✅; на плеере задать `DSIGN_API_TOKEN` 🟡
