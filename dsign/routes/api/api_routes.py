...
def init_api_routes(api_bp, services):
    ...
    def _parse_milli_or_celsius_temp(raw: str) -> float | None:
        """
        sysfs thermal `temp` is usually millidegrees C (e.g. 47234 -> 47.2°C).
        Some broken/virtual zones report bogus signed millidegrees; dividing blindly yields absurd values.
        """
        try:
        ...
        if abs(v) >= 500:
            ...
            c = v
        # Ignore clearly invalid readings (sensor glitch, wrong zone, millikelvin confusion).
    ...
    def _read_cpu_temp_c() -> float | None:
        ...
        for tf in paths:
            try:
                zdir = tf.parent
                typ = (zdir / "type").read_text(encoding="utf-8").strip().lower()
            except Exception:
                typ = ""
        ...
        for tf in preferred + rest:
            try:
                ...
                parsed = _parse_milli_or_celsius_temp(raw)
