...
class ExternalMediaService:
    ...
    playback uses ``ytdl://`` for VK and Rutube so MPV resolves streams on the signage device.
    """
    ...
    def ensure_fresh_playback(self, row: "ExternalMedia", max_age_sec: int = 3600) -> Dict[str, Any]:
        ...
        For VK Video and Rutube, prefer ``ytdl://<canonical page URL>`` so MPV's yt-dlp hook resolves
        streams on the playback device. CDN URLs resolved on the Flask host often embed that host's
        IP in signatures or omit cookies needed for segment fetches — playback then fails on the Pi.
        ...
        # VK / Rutube: resolve via mpv's ytdl_hook on the signage device (same network egress as playback).
