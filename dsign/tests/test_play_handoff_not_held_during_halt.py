"""Stop/Play must not hold play handoff across mpv halt (live: play_lock_timeout)."""
...
def test_play_runner_stop_and_prepare_before_handoff(null_logger, tmp_path):
    ...
    assert order == ["stop", "prepare", "acquire"]


def test_stop_impl_joins_before_handoff(null_logger, tmp_path):
    pm = PlaylistManager(null_logger, None, str(tmp_path), MagicMock(), MagicMock(), MagicMock())
    pm._app = None
    order: list[str] = []
    ...
    real_handoff = pm._acquire_play_handoff

    def _handoff(*, playlist_id: int = 0):
        order.append("handoff")
        assert "join" in order
        return real_handoff(playlist_id=playlist_id)

    pm._acquire_play_handoff = _handoff  # type: ignore[method-assign]

    assert pm._stop_impl(show_idle_logo=True, update_status=True, source="manual") is True
    assert order.index("join") < order.index("handoff")
    assert order.index("handoff") < order.index("clear")
