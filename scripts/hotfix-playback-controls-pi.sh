...
grep -q "NEVER take play handoff" "$RUNTIME_ROOT/services/playlist_management.py" || { echo "MISSING stop without handoff"; fail=1; }
grep -q "Micro handoff" "$RUNTIME_ROOT/services/playback_play.py" || { echo "MISSING play micro handoff"; fail=1; }
